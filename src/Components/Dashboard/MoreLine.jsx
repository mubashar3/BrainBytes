...
const MoreLine = ({ isOpen, onToggle,deleteProject }) => {
    ...
    return (
        <div ref={componentRef} className='relative'>
            <button onClick={handleButtonClick} className={`relative p-1 ${isComponentOpen ? 'rounded-lg bg-[#323940] ' : ''}`}>
                <img className='w-6' src={images.moreLine} alt="more" />
            </button>
            {isComponentOpen && (
                <div className='flex flex-col gap-4 absolute w-32 right-0 border border-gray-700 p-2 rounded-md bg-[#323940] z-10'>
